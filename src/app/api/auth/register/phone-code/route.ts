...
import {
  ...
  sendRegisterPhoneVerificationCode,
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await sendRegisterPhoneVerificationCode(parsed.data.phone)
    return NextResponse.json({
      data: result,
      message: `验证码已发送，请在 ${SMS_VERIFICATION_CODE_EXPIRES_MINUTES} 分钟内完成注册。`,
    })
  } catch (error) {
    ...
    console.error('[auth.register.phone-code] failed to send verification sms:', error)
