...
export async function POST(
  ...
  if (!skill.isActive) {
    return NextResponse.json(
      { error: 'Skill is disabled', code: 'FORBIDDEN_RESOURCE_DISABLED' },
      { status: 403 }
    )
  }
  ...
  if (!skill.organizationId || !scopedOrganizationIds.includes(skill.organizationId)) {
    return NextResponse.json(
  ...
  if (existing) {
    ...
    await recordOperationLog({
      ...
      targetUserId: userId,
      summary: `重新提交 Skill 申请 ${skill.name}`,
      metadata: {
  ...
  await recordOperationLog({
    ...
    targetName: skill.name,
    targetUserId: userId,
    summary: `提交 Skill 申请 ${skill.name}`,
    metadata: {
      resourceType: application.resourceType,
      resourceId: application.resourceId,
      userId,
