...
export default async function SkillsPage() {
  ...
  const typedSkills = skills.map((skill: typeof skills[number]) => ({
    ...skill,
  ...
  const pageDescription = managementMode === 'super_admin'
    ? '集中维护全局 Skill 资产，确保标识符、可见范围和归属信息都保持清楚一致。'
    : managementMode === 'department_admin'
    ? '查看并维护部门范围内的 Skill，减少跨部门资源混杂，保持管理边界明确。'
    : '浏览你当前可访问的 Skill 资源，快速确认名称、包地址和所属范围。'
